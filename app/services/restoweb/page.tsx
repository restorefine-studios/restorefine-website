...
async function fetchWebData() {
  const entry = await getEntry('3nd2PTdZsXmpsPEKdk2PmD');
  if (!entry) return null;

  const heroHeadline = entry.fields?.heroHeadline || 'Web Solutions';
  const heroSubtext = entry.fields?.heroSubtext || 'Digital experiences.';
  const heroImage = entry.fields?.heroImage ? entry.fields.heroImage.map((img: any) => 'https:' + img.fields.file.url) : [];

  const mainServiceTitle = entry.fields?.mainServiceTitle || 'Define Your Web Identity';
  const mainServiceSubtext = entry.fields?.mainServiceSubtext || 'Shaping web through creative strategy.';
  const features = [];
  for (let i = 1; i <= 4; i++) {
  ...
  const overflowImage = entry.fields?.overflowImage ? 'https:' + entry.fields.overflowImage.fields.file.url : '/restowebsignature.svg';

  return {
    hero: { titletop: 'Digital Experiences', titlebottom: 'That Engage', description: heroSubtext, image: heroImage[0] },
...
export default async function RestoWeb() {
