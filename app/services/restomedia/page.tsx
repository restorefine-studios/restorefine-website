...
async function fetchMediaData() {
  ...
  const heroHeadline = entry.fields?.heroHeadline || 'Media Solutions';
  const heroSubtext = entry.fields?.heroSubtext || 'Visual storytelling.';
  const heroImage = entry.fields?.heroImage ? entry.fields.heroImage.map((img: any) => 'https:' + img.fields.file.url) : [];

  const mainServiceTitle = entry.fields?.mainServiceTitle || 'Define Your Media Identity';
  const mainServiceSubtext = entry.fields?.mainServiceSubtext || 'Shaping media through creative strategy.';
  const features = [];
  for (let i = 1; i <= 4; i++) {
  ...
  const overflowImage = entry.fields?.overflowImage ? 'https:' + entry.fields.overflowImage.fields.file.url : '/restomediasignature.svg';

  return {
    hero: { titletop: 'Visual Storytelling', titlebottom: 'That Captivates', description: heroSubtext, image: heroImage[0] },
